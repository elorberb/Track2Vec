...
class EvalRSRecList(RecList):
    ...
    def MR_ITF(self):
        ...
        # grab slice info
        m['class'] = self._y_test['track_id'].values
        # group-by slice and get per-slice mrr
        class_mr = m.groupby('class')['mr'].agg('sum')
        df = pd.DataFrame(class_mr)

        total_num_pred = len(self._y_test)
        track_count = self._y_test.value_counts()
